...
export const getEventsByType = (eventType: Event["eventType"]): Event[] => {
  return events.filter((event) => event.eventType === eventType);
};

export const getWorkshops = (): Event[] => {
  return events.filter((event) => event.eventType === "workshop");
};

export const getRunningClubs = (): Event[] => {
  return events.filter((event) => event.eventType === "running-club");
};

export const getVernissages = (): Event[] => {
...
export const getUpcomingWorkshops = (): Event[] => {
  const today = new Date();
  return events.filter((event) => {
    const eventDate = new Date(event.date.split("/").reverse().join("-"));
